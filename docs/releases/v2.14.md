# Release Notes - All-in-One Model Selection Fixes

## Version: v2.14.9
**Release Date:** 2026-02-20
**Base:** v2.14.8
**Highlights:** Improved duplicate model handling and fixed display issues for multi-panel layouts in All-in-One mode.

---

## All-in-One Improvements

### Fixed Duplicate Selection Logic
- **Prevented Duplicate Models**: You can no longer select the same model in multiple panels. If you select a model that is already in use, the system automatically switches the other panel to a different available model.
- **Preserved Conversation History**: Fixed an issue where resolving duplicate models caused conversation history to be lost in the affected panels.
- **Simultaneous Resolution**: The system now properly handles scenarios where multiple panels might attempt to use the same model simultaneously.

### Fixed Multi-Panel Display Issues
- **Grid Layout Rendering**: Fixed an issue where selected models were not displayed correctly in 4-panel and 6-panel layouts.
- **Display Stability**: Improved internal data handling to prevent blank slots and ensure all panels render the correct models reliably.

### Enhanced Error Handling
- **Model Validation**: The system now verifies that a model is enabled before allowing it to be selected.
- **Insufficient Models Warning**: Added clear, localized error messages (English, Japanese, Simplified Chinese, Traditional Chinese) when there are not enough enabled models to populate the current layout.
- **Layout Change Validation**: Added validation that prevents switching to a larger layout (e.g., 6-panel) if there are insufficient enabled models available.

### Technical Improvements
- Added helper functions for robust model/bot management:
  - `getPanelCount()`: Determines panel count based on the current layout.
  - `resolveEnabledBotIndices()`: Fetches enabled bot indices asynchronously.
  - `normalizeBotsForLayout()`: Normalizes the bot array (removes duplicates, adjusts length).
  - `buildBotsForSelection()`: Builds a valid bot array with auto-swap capability.
